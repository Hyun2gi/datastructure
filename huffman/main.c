...
void huffman_traversal(TreeNode *node,int codes[],int top)
{
    ...
    if(!(node->l) && !(node->r)) //check it is leaf node
    {
        int index = node->data-'a';
        m_bit_size[index] = top;
        for(int i=0;i<top;i++)
        {
            m_LUT[index][i] = codes[i];
        }
    }
}
...
void huffman_encoding(char *str, bits_stream *bits_str)
{
    ...
    for(int i=0;i<strlen(str);i++)
    {
        ...
        {
            bits_str->stream[interval++] = m_LUT[index][j];
        }
    }
...
int huffman_decoding(bits_stream *bits_str, TreeNode *node, char* decoded_str)
{
    ...
    for(int i=0;i<bits_str->length;i++)
    {
        ...
        {
            tmp = tmp->l;
            if(!(tmp->l) && !(tmp->r)) //check it is leaf node
            {
                decoded_str[index_char] = tmp->data;
                index_char++;
                tmp = start;
            }
        }else {
